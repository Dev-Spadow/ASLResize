...
final class Sounds {
	...
	private Sounds()
	{
		aSynthensizerArray329 = new Synthensizer[10];
	}
	...
	private void method242(Buffer buffer)
	{
		for(int i = 0; i < 10; i++)
		{
			...
			if(j != 0)
			{
				...
				aSynthensizerArray329[i] = new Synthensizer();
				aSynthensizerArray329[i].method169(buffer);
			}
		}
	...
	private int method243()
	{
		int j = 0x98967f;
		for(int k = 0; k < 10; k++)
			if(aSynthensizerArray329[k] != null && aSynthensizerArray329[k].anInt114 / 20 < j)
				j = aSynthensizerArray329[k].anInt114 / 20;
		...
		if(j == 0x98967f || j == 0)
			return 0;
		for(int l = 0; l < 10; l++)
			if(aSynthensizerArray329[l] != null)
				aSynthensizerArray329[l].anInt114 -= j * 20;
	...
	{
		int k = method245(i);
		aBuffer_328.currentPosition = 0;
		aBuffer_328.writeDWord(0x52494646);
		aBuffer_328.method403(36 + k);
		aBuffer_328.writeDWord(0x57415645);
		aBuffer_328.writeDWord(0x666d7420);
		...
		aBuffer_328.writeDWord(0x64617461);
		aBuffer_328.method403(k);
		aBuffer_328.currentPosition += k;
		return aBuffer_328;
	}
	...
	private int method245(int i)
	{
		int j = 0;
		for(int k = 0; k < 10; k++)
			if(aSynthensizerArray329[k] != null && aSynthensizerArray329[k].anInt113 + aSynthensizerArray329[k].anInt114 > j)
				j = aSynthensizerArray329[k].anInt113 + aSynthensizerArray329[k].anInt114;
		...
		for(int i2 = 0; i2 < 10; i2++)
			if(aSynthensizerArray329[i2] != null)
			{
				int j2 = (aSynthensizerArray329[i2].anInt113 * 22050) / 1000;
				int i3 = (aSynthensizerArray329[i2].anInt114 * 22050) / 1000;
				int ai[] = aSynthensizerArray329[i2].method167(j2, aSynthensizerArray329[i2].anInt113);
	...
	private static Buffer aBuffer_328;
	private final Synthensizer[] aSynthensizerArray329;
