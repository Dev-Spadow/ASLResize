...
public class RSFont extends Rasterizer2D {
    ...
    public RSFont(boolean TypeFont, String s, FileArchive archive) {
        ...
        Buffer buffer = new Buffer(archive.readFile(s + ".dat"));
        Buffer buffer_1 = new Buffer(archive.readFile("index.dat"));
        buffer_1.currentPosition = buffer.readUShort() + 4;
        int k = buffer_1.readUnsignedByte();
        if (k > 0) {
            buffer_1.currentPosition += 3 * (k - 1);
        }
        for (int l = 0; l < 256; l++) {
            characterDrawXOffsets[l] = buffer_1.readUnsignedByte();
            characterDrawYOffsets[l] = buffer_1.readUnsignedByte();
            int i1 = characterWidths[l] = buffer_1.readUShort();
            int j1 = characterHeights[l] = buffer_1.readUShort();
            int k1 = buffer_1.readUnsignedByte();
            int l1 = i1 * j1;
            fontPixels[l] = new byte[l1];
            if (k1 == 0) {
                for (int i2 = 0; i2 < l1; i2++) {
                    fontPixels[l][i2] = buffer.readSignedByte();
                }

            } else if (k1 == 1) {
                for (int j2 = 0; j2 < i1; j2++) {
                    for (int l2 = 0; l2 < j1; l2++) {
                        fontPixels[l][j2 + l2 * i1] = buffer.readSignedByte();
                    }
    ...
    public static String handleOldSyntax(String text) {
        ...
        text = handleOldSyntax(text, "<3", "<img=653>");
        text = handleOldSyntax(text, ":heart:", "<img=653>");
        return text;
    }
                    ...
                    if (effectString.equals(startEffect)) {
                        ...
                        if (effectString.startsWith(startImage)) {
                            try {
                                ...
                                chatImageId.drawSprite(drawX + 2, (drawY + baseCharacterHeight - iconModY - 13) + 1);
                                drawX += (chatImageId.myWidth + 6);
