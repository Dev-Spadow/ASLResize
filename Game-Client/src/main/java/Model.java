public class Model extends Renderable {
	...
	private static boolean repeatTextures[] = new boolean[4700];
	...
	public static void method460(byte abyte0[], int j) {
		try {
			...
			buffer.currentPosition = abyte0.length - 18;
	...
	public void readOldModel(byte[] data, int modelId) {
		...
		numVertices = buffer.readUShort();
		numTriangles = buffer.readUShort();
		numberOfTexturesFaces = buffer.readUnsignedByte();
		int type_opcode = buffer.readUnsignedByte();
		int priority_opcode = buffer.readUnsignedByte();
		int alpha_opcode = buffer.readUnsignedByte();
		int tSkin_opcode = buffer.readUnsignedByte();
		int vSkin_opcode = buffer.readUnsignedByte();
		int i_254_ = buffer.readUShort();
		int i_255_ = buffer.readUShort();
		int i_256_ = buffer.readUShort();
		int i_257_ = buffer.readUShort();
		int i_258_ = 0;
		...
		triangleColours = new short[numTriangles];
		buffer.currentPosition = i_259_;
		buffer1.currentPosition = i_269_;
		buffer2.currentPosition = i_270_;
		buffer3.currentPosition = i_271_;
		buffer4.currentPosition = i_264_;
		int start_x = 0;
		int start_y = 0;
		int start_z = 0;
		for (int point = 0; point < numVertices; point++) {
			...
			if (vSkin_opcode == 1)
				vertexVSkin[point] = buffer4.readUnsignedByte();

		}
		buffer.currentPosition = i_267_;
		buffer1.currentPosition = i_263_;
		buffer2.currentPosition = i_261_;
		buffer3.currentPosition = i_265_;
		buffer4.currentPosition = i_262_;
		for (int face = 0; face < numTriangles; face++) {
			triangleColours[face] = (short) buffer.readUShort();
			if (type_opcode == 1) {
				int flag = buffer1.readUnsignedByte();
				if ((flag & 0x1) == 1) {
					faceDrawType[face] = 1;
					has_face_type = true;
				} else {
			...
			if (priority_opcode == 255)
				face_render_priorities[face] = buffer2.readSignedByte();

			if (alpha_opcode == 1) {
				face_alpha[face] = buffer3.readSignedByte();
				if (face_alpha[face] < 0)
					face_alpha[face] = (256 + face_alpha[face]);

			}
			if (tSkin_opcode == 1)
				triangleTSkin[face] = buffer4.readUnsignedByte();

		}
		buffer.currentPosition = i_266_;
		buffer1.currentPosition = i_260_;
		int coordinate_a = 0;
		int coordinate_b = 0;
		int coordinate_c = 0;
		int offset = 0;
		int coordinate;
		for (int face = 0; face < numTriangles; face++) {
			int opcode = buffer1.readUnsignedByte();
			if (opcode == 1) {
				coordinate_a = (buffer.readSmart() + offset);
				offset = coordinate_a;
				coordinate_b = (buffer.readSmart() + offset);
				offset = coordinate_b;
				coordinate_c = (buffer.readSmart() + offset);
				offset = coordinate_c;
		...
		buffer.currentPosition = i_268_;
		for (int face = 0; face < numberOfTexturesFaces; face++) {
			texture_type[face] = 0;
			textures_face_a[face] = (short) buffer.readUShort();
			textures_face_b[face] = (short) buffer.readUShort();
			textures_face_c[face] = (short) buffer.readUShort();
		}
	...
	public void readNewModel(byte data[], int modelId) {
		...
		nc1.currentPosition = mainStream_offset;
		nc2.currentPosition = firstStream_offset;
		nc3.currentPosition = secondStream_offset;
		nc4.currentPosition = thirdStream_offset;
		nc5.currentPosition = fourthStream_offset;
		nc6.currentPosition = fifthStream_offset;
		for (face = 0; face < numberOfTexturesFaces; face++) {
	...
	public final void renderAtPoint(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
		...
		int i3 = k2 + l2;
		if (i3 <= 50 || k2 >= (Configuration.OBJECT_CULLING + 1500))
			return;
		...
		try {
			translateToScreen(flag, flag1, i2);
			return;
		} catch (Exception _ex) {
			return;
		}
	...
	private final void translateToScreen(boolean flag, boolean flag1, int i) {
	...
	private final void drawModel(int i) {
		if (outOfReach[i]) {
			method485(i);
			return;
		}
